...
// d(delay): 음수면 마운트 시 이미 진행 중 → 진입하자마자 하트가 어느 정도 올라와 있음
const HEARTS = [
  { l: 10, d: -3.0, dur: 4.2, s: 17 },
  { l: 26, d: -1.2, dur: 5.0, s: 13 },
  { l: 42, d: -4.0, dur: 3.8, s: 20 },
  { l: 57, d: 0.6,  dur: 4.7, s: 14 },
  { l: 72, d: -2.2, dur: 4.1, s: 16 },
  { l: 86, d: 1.4,  dur: 5.3, s: 12 },
  { l: 34, d: -0.6, dur: 4.4, s: 15 },
  { l: 64, d: 2.2,  dur: 3.9, s: 13 },
]

const COLORS = ['#ff6b95', '#ff92b0', '#ff5c86', '#ff7ea3', '#ffa6c0']

// durScale: 애니메이션 길이 배수(클수록 천천히). 큰 영역(상세)에서 속도가 빨라 보이지 않게.
export default function ThemeHearts({ durScale = 1, className = '' }) {
  return (
    <div className={`theme-hearts${className ? ` ${className}` : ''}`} aria-hidden="true">
      {HEARTS.map((h, i) => (
