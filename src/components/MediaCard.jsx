...
export default function MediaCard({ category, info, onClear }) {
  ...
  if (category === 'OTT') {
    ...
    rows.push(['장르', info.genres?.length ? info.genres.join(', ') : <span className="muted">정보 없음</span>])
    ...
    if (info.isbn) {
      // 카카오 책 검색 API(신규, isbn 포함)는 장르·페이지수를 안 줘서 그 자리에
      ...
      // 알라딘 API(구, isbn 없음)로 이미 저장된 기존 카드는 그대로 장르·페이지수 표시
      if (info.genres?.length) rows.push(['장르', info.genres.join(', ')])
