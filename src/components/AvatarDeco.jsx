...
// 겹쳐 그려야 원본 디테일이 살아난다 — 실루엣만으로는 뭉툭한 덩어리로 보임. 실루엣 자체에
// 눈에 띄는 두께의 테두리(stroke)를 같은 path로 둘러 흰색이 테두리 밖으로 삐져나오지 않게 한다.
...
function AngelWing() {
  const wing = (
    <g className="avd-wing-flap">
      <path d={ANGEL_WING_D} fill="#ffffff" stroke="#c7cbe0" strokeWidth="0.6" strokeLinejoin="round" />
...
// 고양이 리본(머리 유형): 앞(front) 레이어, 오른쪽 위 머리 위에 얹힌 리본. 빨간 도형 하나에
// fill(빨강)과 stroke(검정)를 같이 적용 — 검정 선이 항상 이 도형의 실제 가장자리(바깥 테두리 +
// 매듭의 작은 원들)를 따라 균일한 굵기로 그려지고, 빨강은 그 stroke 안쪽 절반에 가려져
// 구조적으로 "테두리보다 뒤"가 된다. 검정을 별도 도형으로 채워 그리던 이전 방식은 두 도형의
// 경계가 살짝 어긋나며 안쪽 선만 가늘어 보이는 문제가 있었다.
...
function KittyRibbon() {
  return (
    <path d={KITTY_RIBBON_RED_D} fill="#ff1832" stroke="#17171b" strokeWidth="1.1" strokeLinejoin="round" />
  )
}
