...
export default function MemberAvatarBtn({ groupId, userId, src, name, size }) {
  const navigate = useNavigate()
  if (!userId || !groupId) return <Avatar src={src} name={name} size={size} />
  const go = (e) => { e.preventDefault(); e.stopPropagation(); navigate(`/groups/${groupId}/members/${userId}`) }
  return (
