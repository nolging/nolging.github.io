...
// deco: { head, face, headTf, faceTf } — 아바타 꾸미기 아이템(선택). 귀는 아바타 뒤(back), 새싹·홍조는 앞(front).
//   headTf/faceTf 는 그룹 프로필 사진에 맞춘 위치·크기·각도 조정값.
...
export default function Avatar({ src, name = '?', size = 34, deco }) {
  ...
  return (
    <span className="avatar" style={{ width: size, height: size, fontSize: size * 0.42 }}>
      {hasDeco && <AvatarDeco {...deco} layer="back" />}
      <span className="avatar-face">{src ? <img src={src} alt={name} /> : initial}</span>
      {hasDeco && <AvatarDeco {...deco} layer="front" />}
    </span>
  )
}
