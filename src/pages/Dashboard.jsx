          ...
          {filtered.map((g) => {
            ...
            return (
              <Link key={g.id} to={`/groups/${g.id}`}
                className={`group-tile group-card ${isMember ? '' : 'not-joined'} ${premium ? 'premium' : ''} ${friend ? 'friend' : ''} ${g.deco_theme === 'heart' ? 'tile-heart' : ''} ${g.deco_theme === 'bubble' ? 'tile-bubble' : ''} ${g.deco_theme === 'firework' ? 'tile-fw' : ''} ${g.deco_theme === 'waterpark' ? 'tile-waterpark' : ''} ${g.deco_theme === 'star' ? 'tile-star' : ''}`}>
