...
// 너무 좁았다(약 90ms). k=2(여유 충분하지만 "너무 느림") → k=1.4(여유 약 280ms) → k=1.25(여유
// 약 200ms) 순으로 낮춰 왔는데, 하강(GRAVITY_DOWN)이 빨라진 만큼 상승도 더 빨라야 자연스럽게
// 느껴진다는 피드백을 받아 k=1.15로 한 번 더 낮췄다(여유 약 170ms — v→v0·S/k, g→g0·S/k² 로
// 스케일하면 높이(v²/2g)는 그대로, 시간(v/g)만 k배가 된다). 떨어질 때는 더 큰 중력을 따로
// 적용해 하강만 더 빠르고 경쾌하게.
const GRAVITY_UP = (0.0022 * SPRITE_SCALE) / 1.15 ** 2   // px / ms^2
const GRAVITY_DOWN = GRAVITY_UP * 2                       // px / ms^2 — 하강은 더 빠르게
const JUMP_V = (-0.62 * SPRITE_SCALE) / 1.15   // px / ms (음수 = 위)
