-- =============================================================
--  미니게임 통합 스키마 (tarot / puzzle)
--  이 파일은 예전에 아래 3개의 개별 파일로 나뉘어 있던 SQL 을 하나로 합친 것입니다.
...
--
--  ⚠️ 로또(냥피또)는 원래 이 파일(구 lotto.sql)에 있었으나, 미니게임이 아니라 상점 아이템
--  (store_items.id='lotto')의 사용/효과 로직에 더 가까워 schema-store-items.sql 로 옮겼다
--  (테이블/RLS/함수/GRANT/pg_cron 전부). 이 파일에는 더 이상 로또 관련 내용이 없다.
--
--  ⚠️ 칭찬 스티커도 원래 이 파일에 있었으나, 미니게임이 아니라서 schema-praise.sql 로
--  옮겼다(테이블/RLS/함수/시드/GRANT 전부. praise_place() 는 더 예전부터 이미
--  schema-notifications.sql 에 있었음). 이 파일에는 더 이상 칭찬 스티커 관련 내용이 없다.
--
--  ⚠️ 이미 운영 중인(live) 프로덕션 DB에는 원본 파일들이 각각 순서대로 이미 적용되어
