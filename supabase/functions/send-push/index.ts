...
Deno.serve(async (req) => {
  try {
    ...
    } else if ((record.type === 'qworkshop_comment' || record.type === 'qworkshop_reply' || record.type === 'qworkshop_post' || record.type === 'mention') && record.group_id && record.qworkshop_post_id) {
      url = `/groups/${record.group_id}/qworkshop/${record.qworkshop_post_id}` // 새 물음표/댓글/답글/멘션 → 물음표 상세
    ...
    const body = JSON.stringify({
      ...
      // 서비스워커가 '앱을 보고 있으면 표시 생략' 판단에 사용(오류 리포트 채팅 등)
