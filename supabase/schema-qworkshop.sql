-- =============================================================
--  물음표 공방(question workshop) — 통합 스키마
--
--  아래 3개 파일에 걸쳐 점진적으로 추가돼 온 물음표 공방 기능을 하나로
--  합친 파일이다(저장소 정리 작업의 일부로 생성됨):
--   · question-workshop.sql        — 개설(구매→그룹에 사용→메뉴 노출) 로직
--   · qworkshop-content.sql        — 실제 콘텐츠: 물음표(vs/고르기/문답) 글/답변/댓글
--   · qworkshop-new-post-notif.sql — 새 물음표 알림(qworkshop_create_post 재정의)
--
--  각 파일이 같은 함수를 여러 번 재정의한 경우 이 파일에는 최종 버전만 담았다
--  (예: qworkshop_create_post 는 새 물음표 알림을 보내는 최신 버전).
--
--  schema.sql + schema-v2.sql 적용 이후라면 이 파일 하나만 실행해도 물음표 공방
--  기능 전체가 갖춰진다(새 환경 셋업/재해복구용). 이미 위 3개 파일을 순서대로
--  실행해 온 운영 DB에는 다시 실행할 필요 없다 — 문서화 목적으로 존재하는 파일.
-- =============================================================

-- ── 테이블 ────────────────────────────────────────────────

-- 그룹당 1행 = 개설됨(비밀 게시판의 group_boards 와 동일 패턴)
...
-- 댓글: task_comments 와 동일 패턴(실명 + 답글 1단계 + @멘션). 비밀 게시판(익명, 멘션 없음)과는 다름.
...
-- ── RLS ───────────────────────────────────────────────────
-- 직접 접근은 전면 차단(비밀 게시판과 동일한 강한 패턴) — 모든 접근은 아래 security definer RPC 로만.
-- 정책(policy)은 별도로 두지 않는다: RLS 만 켜고 어떤 policy 도 만들지 않으면
-- 테이블 소유자/service_role 을 제외한 모든 직접 접근이 차단된다.
alter table public.group_qworkshops   enable row level security;
...
-- 개설 여부 조회 — 그룹 멤버만 확인 가능(데이트/놀이터 메뉴 노출 판단용)
...
-- 아이템을 사용할 수 있는(프리미엄 + 미개설) 내 그룹 목록
...
-- 개설: 아이템 1개 소모 → group_qworkshops 에 1행 추가
...
-- ── 함수: 권한 헬퍼(비밀 게시판의 board_access/board_can_manage 와 동일 패턴) ──
...
-- ── 함수: 답변/선택(게이팅) — task_reviews_view 와 완전히 같은 패턴 ──
...
-- ── 트리거 ────────────────────────────────────────────────
-- updated_at 자동 갱신(board_touch 재사용 — schema.sql/schema-v2.sql 에 이미 정의됨)
