-- =============================================================
--  프로필 꾸미기(얼굴 장식) 추가: 반창고
--   · 오른쪽 볼에 데일밴드를 붙인 모양. 눈/볼 위치에 그려지는 face 슬롯
--     (홍조·빠직·선글라스와 같은 슬롯 → 하나만 장착)
--   · 우선 관리자 전용 노출(admin_only = true)
--  적용: Supabase SQL Editor 에 그대로 실행.
-- =============================================================

-- 슬롯 구분: 반창고도 face 로 인식. 그 외 deco-* 는 head.
create or replace function public.deco_slot(p_item_id text)
returns text language sql immutable as $$
  select case when p_item_id in ('deco-blush', 'deco-anger', 'deco-pixel-shades', 'deco-alien-shades', 'deco-bandage')
                then 'face'
...
select id, name, price, emoji, description, premium, tier, admin_only, sort_order,
  ...
  from public.store_items where id = 'deco-bandage';
